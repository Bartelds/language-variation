...
int main (int argc, char *argv [])
{
	...
	*ii,
    ...
    for (i = 0; i < n_names; i++)
        if (names [i].v != -1 && ! ((names [i].v > 0) && (names [i].v & 64)))
	    names [i].usage = USE;
...
void syntax ()
{
    fprintf (
        stderr,
        "\n"
        "Plain Map Drawing, Version " my_VERSION "\n"
        "(c) P. Kleiweg 1998 - 2010\n"
        "\n"
        "Usage: %s [-o filename] configfile\n"
        "\n"
        "\t-o : output file\n"
        "\n",
        programname
